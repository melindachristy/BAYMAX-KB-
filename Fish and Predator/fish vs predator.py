...
fishh = pygame.image.load("fish.png")
fishrect = fishh.get_rect()

enemy = pygame.image.load("predator.png")
...
#Agar fish berkumpul
    for fish in fishes:
        ...
        for otherFish in fishes:
            ...
            if distance < 500: #dalam radius 500 fish baru mendekati fish lain
...
#predator mendekati fish
    for predator in predators:
        closePredators = []
        for fish in fishes:
            distance = fish.distance(predator)
            if(distance < 200): #dalam radius 200 predator baru mendekati fish
